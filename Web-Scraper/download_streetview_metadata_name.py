...
def geocode_stop_name(stop_name):
    url = "https://maps.googleapis.com/maps/api/geocode/json"
    params = {
        "address": f"{stop_name}, Durham, NC",
        "key": api_key
    }
...
for _, row in stops.iterrows():
    ...
    geocoded = geocode_stop_name(stop_name)

    if geocoded is None:
        print(f"Skipping {stop_code}: could not geocode stop name")
        continue

    bus_lat, bus_lon = geocoded
    print(f"{stop_code}: geocoded '{stop_name}' to ({bus_lat}, {bus_lon})")
